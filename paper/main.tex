...
. Learning procedure took advantage of  model and data-parallelism in a CPU-based cluster environment. This network gave an error rate of  6.67\%  on  ILSVRC 2014 data set. This experiments shows that use of existing dense  blocks to  build the sparse structure can improve the  performance of convolutional networks. Even if higher depth will increase the accuracy , this system will end up with implementing large number of hidden layers to increase accuracy for a high frequency input. 
...
\begin{table*}[ht]
 \begin{tabular}{ | p{3cm} | l | p{1.4cm} |p{1.3cm} |p{1cm} |p{1.2cm} |p{1.1cm} |p{6cm}|}
    \hline
Team	&Year	&Data Augmentation	&scalable over network	&Time taken	&Hardware	&Error on  ILSVRC & Observations
\\ 
\hline
Ren Wu et al.\cite{Wu2015}	&2015	&aggressive	&Yes	&8.8 hours	&multi-GPU cluster	&5.98\% &Aggressive augmentation  will make the problem depend on data set\\
\hline
Karen Simonyan and Andrew Zisserman.\cite{Arge2015}	&2014	&minimum	&No	&3 weeks	&multi-GPU	&6.80\% &Effect of small convolution filter in low frequency domain need to be studied\\
\hline
Christian Szegedy and et al.\cite{Szegedy}	&2014	&minimum	&Yes	&more than a week	&CPU cluster	&6.67\% &Inserting more layers will make system depends on the data set \\
\hline
Kaiming He et al.\cite{He2014}	&2014	&minimum	&No	&4 weeks	&single GPU	&8.06\% &Good method, But higher time complexity\\
\hline
Matthew D. Zeiler and Rob Fergus \cite{Zeiler2013}	&2013	&minimum	&No	&12 days	&single GPU	&11.74\% & Visualizations are not possible in very deep network\\
\hline
Min Lin et al.\cite{Lin2013}	&2013	&minimum	&No	&Not reported	&single GPU	&12.95\% &Inserting more layers will over-fit the system \\
\hline
